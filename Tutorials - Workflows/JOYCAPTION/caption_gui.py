...
class CaptionApp(QWidget):
    ...
    def initUI(self):
        ...
        # Add QLabel and QTextEdit for Generated Caption
        right_panel.addWidget(QLabel("Generated Caption:"))
        self.generated_caption_text = QTextEdit()
        self.generated_caption_text.setReadOnly(False)  # Allow editing
        self.generated_caption_text.setPlaceholderText("Generated caption will appear here...")
        right_panel.addWidget(self.generated_caption_text)

        # Add QPushButton to Save Edited Caption
        self.save_caption_button = QPushButton("Save Edited Caption")
        self.save_caption_button.clicked.connect(self.save_edited_caption)
        self.save_caption_button.setEnabled(False)  # Disabled until a caption is generated
    ...
    def display_selected_image(self, item):
        ...
        if not pixmap.isNull():
            ...
            # Clear the generated caption textbox and disable the save button
    ...
    def generate_captions(self):
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                # Display the generated caption in QTextEdit
                self.generated_caption_text.setText(caption)
                self.save_caption_button.setEnabled(True)  # Enable the save button
    ...
    def save_edited_caption(self):
        ...
        if not edited_caption:
            QMessageBox.warning(self, "Empty Caption", "The edited caption is empty. Please enter some text.")
            return

        # Determine the file path to save the edited caption
        if self.selected_image_path:
            caption_file = self.selected_image_path.with_suffix('.txt')
        elif self.single_image_path:
            caption_file = self.single_image_path.with_suffix('.txt')
        else:
            QMessageBox.warning(self, "No Image Selected", "Please select an image to save its caption.")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Save Error", f"An error occurred while saving the caption: {e}")
    ...
    def resizeEvent(self, event):
        ...
        if self.selected_image_path and self.selected_image_label.pixmap():
            ...
            if not pixmap.isNull():
                # Rescale the pixmap to fit the label size while preserving aspect ratio
                scaled_pixmap = pixmap.scaled(
                    self.selected_image_label.size(),
                    Qt.KeepAspectRatio,
                    Qt.SmoothTransformation
